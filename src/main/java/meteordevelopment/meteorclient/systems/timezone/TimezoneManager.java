...
public class TimezoneManager extends System<TimezoneManager> {
    ...
    private void updateTimezone() {
        try {
            ...
            if (showTimezoneInfo.get()) {
                ...
                MeteorClient.LOG.info("Timezone: {} | Local Time: {} | UTC Time: {} | {}", 
                    currentZone.getId(),
            ...
            MeteorClient.LOG.error("Invalid timezone: {}, falling back to UTC", timezone.get(), e);
    ...
    public void autoDetectSystemTimezone() {
        try {
            ...
            if (useLocationDetection.get() && InternetTimeSync.get() != null && InternetTimeSync.get().isLocationDetected()) {
                ...
                if (detectedTimezone != null) {
                    try {
                        ...
                        MeteorClient.LOG.info("Auto-detected timezone from location: {} ({})", 
                            detectedTimezone, InternetTimeSync.get().getDetectedLocation());
                        return;
                    } catch (Exception e) {
                        MeteorClient.LOG.warn("Invalid detected timezone: {}, falling back to system timezone", detectedTimezone);
                    }
                }
            }
            ...
            MeteorClient.LOG.info("Auto-detected system timezone: {}", systemZone.getId());
        } catch (Exception e) {
            MeteorClient.LOG.error("Failed to auto-detect timezone, using UTC", e);
            timezone.set("UTC");
        }
    }
