...
package meteordevelopment.meteorclient.systems.timezone;
...
public class InternetTimeSync extends System<InternetTimeSync> {
    ...
    public void syncInternetTime() {
        ...
        CompletableFuture.supplyAsync(() -> {
            try {
                    ...
                    try {
                        ...
                        if (response.statusCode() == 200) {
                            ...
                            if (serverTime != null) {
                                ...
                                if (showSyncInfo.get()) {
                                    MeteorClient.LOG.info("Internet time sync successful from {}: {}", 
                        ...
                        if (showSyncInfo.get()) {
                            MeteorClient.LOG.warn("Failed to sync from {}: {}", server, e.getMessage());
                        }
                    }
                }
                ...
                MeteorClient.LOG.error("Internet time sync failed", e);
                return false;
            }
        }).thenAccept(success -> {
            if (!success && showSyncInfo.get()) {
                MeteorClient.LOG.warn("All internet time servers failed, using system time");
            }
        });
    }
    ...
    public void detectLocation() {
        ...
        CompletableFuture.supplyAsync(() -> {
            try {
                    ...
                    try {
                        ...
                        if (response.statusCode() == 200) {
                            ...
                            if (locationInfo != null) {
                                ...
                                if (showSyncInfo.get()) {
                                    MeteorClient.LOG.info("Location detected: {} | Timezone: {}", 
                                ...
                                if (TimezoneManager.get() != null && detectedTimezone != null) {
                                    try {
                                        ...
                                        MeteorClient.LOG.info("Auto-updated timezone to: {}", detectedTimezone);
                                    } catch (Exception e) {
                                        MeteorClient.LOG.warn("Invalid detected timezone: {}", detectedTimezone);
                                    }
                                }
                        ...
                        if (showSyncInfo.get()) {
                            MeteorClient.LOG.warn("Failed to detect location from {}: {}", service, e.getMessage());
                        }
                    }
                }
                ...
                MeteorClient.LOG.error("Location detection failed", e);
                return false;
            }
        });
    }
    ...
    private Instant parseTimeFromResponse(String response, String server) {
        try {
            ...
            MeteorClient.LOG.warn("Failed to parse time from response: {}", e.getMessage());
        }
        return null;
    }
    ...
    private LocationInfo parseLocationFromResponse(String response, String service) {
        try {
            ...
            MeteorClient.LOG.warn("Failed to parse location from response: {}", e.getMessage());
        }
        return null;
    }
