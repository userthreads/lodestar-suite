...
public class MeteorTextHud {
    ...
    static {
        ...
        MODULE_ENABLED = addPreset("Module enabled", "AutoClicker: {meteor.is_module_active(\"auto-clicker\") ? #2 \"ON\" : #3 \"OFF\"}", 0);
        MODULE_ENABLED_WITH_INFO = addPreset("Module enabled with info", "AutoClicker: {meteor.is_module_active(\"auto-clicker\") ? #2 \"ON\" : #3 \"OFF\"} #1{meteor.get_module_info(\"auto-clicker\")}", 0);
        WATERMARK = addPreset("Watermark", "Lodestar #1{meteor.version}");
    }
