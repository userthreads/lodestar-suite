...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(LodestarSuite.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(LodestarSuite.MOD_ID, file.getName());
            ...
            LodestarSuite.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    LodestarSuite.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    LodestarSuite.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            LodestarSuite.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
