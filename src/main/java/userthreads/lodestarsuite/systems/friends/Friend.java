...
package userthreads.lodestarsuite.systems.friends;

import com.mojang.util.UndashedUuid;
import userthreads.lodestarsuite.LodestarSuite;
import userthreads.lodestarsuite.utils.misc.ISerializable;
import userthreads.lodestarsuite.utils.network.FailedHttpResponse;
import userthreads.lodestarsuite.utils.network.Http;
import userthreads.lodestarsuite.utils.render.PlayerHeadTexture;
import userthreads.lodestarsuite.utils.render.PlayerHeadUtils;
    ...
    public void updateInfo() {
        ...
        if (id != null) {
            res = Http.get("https://sessionserver.mojang.com/session/minecraft/profile/" + UndashedUuid.toString(id))
                .exceptionHandler(e -> LodestarSuite.LOG.error("Error while trying to connect session server for friend '{}'", name))
        ...
        if (res == null || res.statusCode() != 200) {
            res = Http.get("https://api.mojang.com/users/profiles/minecraft/" + name)
                .exceptionHandler(e -> LodestarSuite.LOG.error("Error while trying to update info for friend '{}'", name))
                .sendJsonResponse(APIResponse.class);
        }
